...
class BusPassenger(Printable):
    ...
    parameters = 'name', 'source', 'destination', 'time'
    ...
    def __init__(self, name, source, destination, time):
        ...
        if not isinstance(time, int):
            raise TypeError('time should be integer number.')
...
class BusStop(Printable):
    ...
    def update_animation(self, current_time):
        ...
        waiting_time = 0
        print("----------")
        for index, stop_passenger in enumerate(self.passengers):
            waiting_time += current_time - stop_passenger.time

        if waiting_time == 0:
            pass
        else:
            waiting_time = waiting_time / len(self.passengers)

        color_index = int(waiting_time // 20)
        ...
        self.queue_line.set_color(color_ramp[color_index])
...
class Bus(Printable):
    ...
    """
    parameters = 'name', 'position', 'direction', 'passengers', 'maxCapacity, Speed'

    def __init__(self, name, position, direction, passengers, maxcapacity, Speed):
        ...
        if not all(isinstance(p, BusPassenger) for p in passengers):
            raise TypeError('passengers should be a list of BusPassenger')
        if not isinstance(maxcapacity, int):
            raise TypeError('maximum capacity must be in integer value')
        if not isinstance(Speed, int):
            raise TypeError('Speed of bus must be in integer value')
...
class BusNetwork(Printable):
    ...
    def update_animation(self, curtime):
        """Update matplotlib animation for axes ax"""
        ...
        # Redraw bus stops
        for stop in self.stops:
            patches += stop.update_animation(curtime)
